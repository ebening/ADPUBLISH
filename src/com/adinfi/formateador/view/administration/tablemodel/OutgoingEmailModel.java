 ...
 */
public class OutgoingEmailModel extends AbstractTableModel {
    ...
    private final String[] columnNames = new String[]{
        ColumNamesConstants.CHECK.getCaption(), ColumNamesConstants.ID.getCaption(), ColumNamesConstants.EMAIL.getCaption(),ColumNamesConstants.DATEMODIFY.getCaption(), ColumNamesConstants.STATUS.getCaption()
    };
    ...
    public OutgoingEmailModel(List<OutgoingEmailBO> list) {
        data = new Object[list == null ? 0 : list.size()][];
        for (int i = 0; i < list.size(); i++) {
            OutgoingEmailBO row = list.get(i);
            data[i] = new Object[]{
                row.isCheck(), row.getIdOutgoingEmail(), row.getEmail(), row.getDatemodify(), row.isStatus()
            };
        }
    }
